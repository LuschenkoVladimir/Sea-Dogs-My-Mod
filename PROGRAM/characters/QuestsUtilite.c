...
void SetQuestAboardCabinDialog(ref refChar)
{
	...
	if (CheckAttribute(refChar, "CaptanId"))
	{
		...
		if (refChar.CaptanId == "DjekHigens")//Вызывающий доверие, Джек Хигенс
		{
		    LAi_SetCheckMinHP(refChar, 10, true, "QuestAboardCabinDialog");  // сколько НР мин
			refChar.Dialog.FileName = "NewPapka/DjekHigens.c";
			refChar.Dialog.CurrentNode = "Djek_abordage"; //даем абордажную ноду		
		}
