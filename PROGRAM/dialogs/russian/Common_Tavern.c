	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (CheckAttribute(pchar, "questTemp.Sharlie") && pchar.questTemp.Sharlie == "skiper" && npchar.city == "FortFrance")
			{
				...
				Link.l1 = "Слушай, "+npchar.name+", я так понимаю, стычки между посетителями у тебя не такое уж редкое дело.";
...
*/				
	...
	////////////////////////////////////////////////////////=============================================///////////////////////////////////////////////////////	
		...
		//--> Бремя гасконца
		case "Sharlie_crew":
			dialog.text = "Ваш офицер, месье, уже обращался ко мне с этим вопросом. К сожалению, в данный момент я ничем не могу вам помочь - люди, которых я знаю, к вам на службу точно не пойдут.";
			link.l1 = "Очень жаль.";
			link.l1.go = "exit";
		...
		case "Sharlie_skiper":
		    dialog.text = "Ну, не то чтобы часто...(понизив голос) есть тут один головорез, Франк Дюгари. Как напьётся, к нему лучше не подходить. Мрачный тип. После того как он отправил на тот свет не один десяток человек... и из-за чего? Чего не скажешь в подпитии? Так вот после всех его безобразий в мою таверну даже сократился приток посетителей. Представляете, месье? Но я то тут при чём? Как я могу отказать ему в кружке пива? Я ещё не хочу на тот свет. Но после очередного дебоша со смертоубийством, который он учинил несколько дней назад, он перестал появляться. И я каждый день молюсь всем святым, которых знаю, чтобы больше духу его не было в моём заведении. Так что отдыхайте спокойно, месье.";
			link.l1 = "А не знаешь, как можно найти этого Дюгари?";
			link.l1.go = "Sharlie_skiper1";
		break;

		case "Sharlie_skiper1":
		    dialog.text = "Не знаю и знать не хочу, месье. Это очень, очень опасный человек. Ну если только вы так уверены в себе....Я бы не поскупился за то, чтобы он больше никогда не появился на пороге этого заведения. А ещё лучше вообще ни на каком пороге. Вы меня понимаете? Но только если что, то этого разговора не было.";
			link.l1 = "Но как же я его найду?";
			link.l1.go = "Sharlie_skiper2";
		break;

		
		case "Sharlie_skiper2":
			dialog.text = "Я действительно понятия не имею, где его искать. Но личность он в Сен-Пьере достаточно известная, поэтому просто поспрашивайте жителей, может кто-нибудь что-нибудь да знает. И берегите себя, месье.";
			link.l1 = "Хорошо, воспользуюсь твоим советом, "+npchar.name+"";
			link.l1.go = "exit";
			pchar.questTemp.Sharlie = "findskiper";
			AddQuestRecord("Sharlie", "5-27");
			//сгенерим наводчика
