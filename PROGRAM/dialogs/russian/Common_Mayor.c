	...
	switch (Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (CheckAttribute(pchar, "questTemp.Sharlie.Citcount") && npchar.city == "FortFrance")
			{
				...
				link.l1 = "Здравствуйте, Ваша Светлость. Я только что прибыл из Парижа. Я брат Мишеля де Монпе, который, по моим сведениям, содержится на базе Мальтийского ордена. Ступив на берег вверенной вам Его Величеством провинции, я подвергся подлому нападению.";
                link.l1.go = "Sharlie";
...
//<-- осады homo
		...
		//Jason --> Бремя гасконца
		case "Sharlie":
			dialog.text = "Что-что? Нападению? Где ваши рекомендации?";
			link.l1 = "Клянусь вам, Ваша Светлость, они у меня были, но мерзавцы, напавшие на меня, отобрали их...";
			link.l1.go = "Sharlie_1";			
		break;
		
		case "Sharlie_1":
			dialog.text = "Вот как? Вот что, сударь: вы мне кажетесь очень подозрительным, и я вас должен арестовать для дальнейшего разбирательства.";
			link.l1 = "Но, Ваша Светлость!";
			link.l1.go = "exit";
			AddDialogExitQuest("DeMor_46"); 
