...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    switch (Dialog.CurrentNode)
	{
		case "quests":
			...
			// Мой квест
			if(CheckAttribute(npchar, "quest.MyQ"))
			{
            link.l1 = "Не подскажете, где я могу найти Диего Эспинозу?";
            link.l1.go = "diego1";
            }			
		break;
		
		case "diego1":
		dialog.text = "Советую побеседовать вон с тем господином за круглым столиком.";
		link.l1 = "Спасибо.";
		link.l1.go = "diego2";
		break;
		
		case "diego2":
		DeleteAttribute(npchar, "quest.MyQ");
		pchar.quest.MyQ1.win_condition.l1 = "location";
		pchar.quest.MyQ1.win_condition.l1.location = "Charles_tavern";
		pchar.quest.MyQ1.win_condition = "MyQ2";
		dialogexit();
		break;
