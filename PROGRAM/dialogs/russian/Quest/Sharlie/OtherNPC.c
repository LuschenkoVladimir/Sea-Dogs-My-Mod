...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Startsailor":
			dialog.text =  "Просыпайтесь, месье де Мор... Наш корабль уже в порту Сен-Пьера!";
		...
		case "Startsailor_2":
			dialog.text = "Вы, должно быть, важная птица, месье. Доставить вас в целости и сохранности до этих чёртовых островов было распоряжением самого Высокопреосвященства. Надеюсь, пребывание на моём судне не было для вас в тягость.";
			link.l1 = "(Шарль встаёт, слегка покачиваясь) Ну что вы, месье Артуа. Конечно не сравнится с королевскими покоями в Лувре, ха-ха, но у меня было кое-что, что несколько скрашивало моё уединение в этой каюте (указывает на груду бутылок от дорогого парижского вина).";
			link.l1.go = "Startsailor_3";			
		break;
		
		case "Startsailor_3":
			dialog.text = "Да, вижу, вы тут времени не теряли, ха-ха. Кровь Христова! Двадцатилетнее бургундское! Теперь я понимаю, что за багаж мы грузили в Гавре! Ха-ха.";
			link.l1 = "Его Преосвященство - великий человек и весьма щедр со своими слугами (продолжая еле заметно пошатываться). Так что не волнуйтесь, месье Артуа, обратно ничего выгружать уже не придётся. (Показывая на ещё не тронутые бутылки) Судьба благоволит вам и вашей замечательной команде, и наше путешествие закончилось раньше, чем это вино. Пейте его на обратном пути во славу Кардинала.";
			link.l1.go = "Startsailor_4";			
		break;
		
		case "Startsailor_4":
			dialog.text = "О, месье де Мор, это воистину самый удачный рейс за всю мою жизнь, ха-ха. Я - ваш вечный должник. Но не забудьте свои остальные вещи...ха-ха";
			link.l1 = "Да, я уже готов... Где моя шпага? Ах, вот... Ладно, идём.";
		...
		case "Startsailor_6":
		AddDialogExitQuest("DeMor_1");                     //Моё добавление ворчания Шарля
		AddDialogExitQuest("BasterEvent_1");                        // Квест в Бастере
		...
		TakeNItems(pchar, "potion1", 40);                 //Дадим Шарлю дополнительный набор пистолетов
		...
		case "Sharlie_maltie_2":
			DialogExit();
			AddDialogExitQuest("DeMor_172");  // переносим Виктиме и Мальтийца в подземелье
		...
		case "Sharlie_maltie_3":
			...
			link.l1 = "Я затем и прибыл в эту глушь, чтобы помочь брату. Надеюсь, мне удастся хоть что-то сделать.(Когда Мальтиец поворачивается, чтобы показать дорогу, вонзает ему нож в спину)";
			link.l1.go = "Sharlie_maltie_4";
		break;

        case "Sharlie_maltie_4":
		DialogExit();
		AddDialogExitQuest("DeMor_18"); 
		break;

		
		case "Sharlie_maltie_41":
			DialogExit();
		...
		case "Sharlie_sailor":
			...
			link.l1 = "Да. У меня есть в собственности корабль, но нет команды. Мой офицер Паскаль Жаке посоветовал мне поговорить с тобой на эту тему - ты и твои товарищи, вроде, как были уволены с какого-то судна и вам нужна работа...";
		...
		case "skipertalker_1":
			...
			link.l1 = "Послушайте, "+GetAddress_FormToNPC(NPChar)+", Я ищу человека по имени Франк Дюгари. Не знаете такого?";
			link.l1.go = "skipertalker_2";
		break;
		
		case "skipertalker_2":
			dialog.text = "(Осторожно оглядываясь) Зачем вам этот мерзавец? Это он прикончил моего брата в местной таверне пару месяцев назад. Да, Пьер сам был не подарком, когда выпьет...упокой его душу. Но этот негодяй даже не извинился перед нашей семьёй, я уж не говорю о компенсации. Но теперь справедливость восторжествовала! Ему долго всё сходило с рук, но после очередного убийства несколько дней назад он был арестован и брошен в тюрьму.";
			link.l1 = "Что же случилось, что власти наконец-то обратили внимание на этот беспредел?";
			link.l1.go = "skipertalker_3";
		break;
		
		case "skipertalker_3":
			dialog.text = "Это мне неизвестно, месье. Но я только надеюсь, что теперь он за всё заплатит сполна!";
			link.l1 = "Полностью с вами согласен, месье, и спасибо за то, что раскрыли мне глаза, какой это негодяй.";
			link.l1.go = "exit";
			LAi_CharacterDisableDialog(npchar);
			npchar.lifeday = 0;
			pchar.questTemp.Sharlie = "jailskiper";
			//AddQuestRecord("Sharlie", "8");
			AddQuestRecord("Sharlie", "5-28");
		...
		case "Folke":
			...
			link.l1 = "Приветствую, месье Дюгари. Последнее время вы стали почти что местной легендой. Но буду краток. Вы любите и умеете убивать. Работая на меня вы сможете совместить приятное с полезным, так как будете получать весьма приличное жалование.";
			link.l1.go = "Folke_1";
		break;
		
		case "Folke_1":
			dialog.text = "Вы хотите сказать, что вытащили меня отсюда, чтобы взять к себе на службу? (саркастически) А я уж рассчитывал, что вы прониклись чувством сострадания и христианского прощения.";
			link.l1 = "Я полагал, что вы будете более рады своему спасению, месье Дюгари.";
			link.l1.go = "Folke_2";
		break;
		
		case "Folke_2":
			dialog.text = "Как вас зовут, месье?";
			link.l1 = "Моё имя Шарль де мор. Я недавно прибыл из Парижа. Здесь по делу своего брата, и с первых дней пребывания на Антильских островах понял, что не обойтись без людей мастерски владеющих шпагой.";
			link.l1.go = "Folke_3";
		break;
		
		case "Folke_3":
			dialog.text = "Хорошо, месье де Мор, я подумаю над ваши предложением.";
			link.l1 = "Подумаете? Ну что ж....Меня предупредили, что если вы возьмётесь за старое, то тут же вернётесь обратно, и тогда вас уже никто не спасёт от виселицы. Поэтому в ваших же интересах, если вы подумаете, будучи почётным гостем на моём корабле. На выходе из тюрьмы вас встретит пара моих матросов, и вы на него отправитесь.";
			link.l1.go = "Folke_4";
		break;
		
		case "Folke_4":
			dialog.text = "В любом случае благодарю за своё спасение, месье де Мор.";
			link.l1 = "...";
			link.l1.go = "Folke_5";
		break;
		...
		case "Folke_5":
			...
			pchar.quest.Folke5.win_condition.l1 = "location";
	        pchar.quest.Folke5.win_condition.l1.location = "fortFrance_town";
	        pchar.quest.Folke5.win_condition = "FirstOut_1";
		break;


		case "DugariSail_1":
			dialog.text = "Месье, человек, которого мы должны были сопровождать на ваш корабль, сбежал.";
			link.l1 = "Что? Уже? Он же был без оружия. А вы куда смотрели?! Я же вас предупреждал глаз с него не спускать.";
			link.l1.go = "DugariSail_2";
		break;


		case "DugariSail_2":
			dialog.text = "Да, месье...Но этот чёрт со связанными руками ухитрился вырубить пару матросов и нырнуть в подземелье. Остальные побежали за ним и теперь его там ищут.";
			link.l1 = "Болваны! Идём, хотя я сомневаюсь, что теперь мы сможем его найти.";
			link.l1.go = "DugariSail_exit";
		break;


		case "DugariSail_exit":
			DialogExit();
			AddDialogExitQuest("FirstOut_3");
		break;
		...
		case "Seabattle_pirate_1":
			dialog.text = "Позвольте представиться - "+GetFullName(npchar)+". Меня послал к вам Чезаре Крейг. Вы, должны его знать - это хозяин таверны в Ле Франсуа. Он сказал, что вы ожидаете важного сообщения, и у него есть нужная вам информация.";
			link.l1 = "Хорошо, передайте ему, что как только корабль будет готов к отплытию, мы немедленно отправимся к берегам пиратского поселения на другой стороне острова.";
			link.l1.go = "Seabattle_pirate_2";
		break;
		
		case "Seabattle_pirate_2":
			dialog.text = "Да, он просил поторопиться, отправляться надо немедленно, потом в этом уже не будет никакого смысла. И ещё, месье де Мор. Чезаре сказал, что формальным поводом для вашего появления в Ле Франсуа должна стать доставка партии вина на Гваделупу. Он сейчас как раз активно этим занимается, поэтому ваш визит не вызовет никаких подозрений.";
			link.l1 = "Что-то ещё?";
			link.l1.go = "Seabattle_pirate_3";
		break;
		
		case "Seabattle_pirate_3":
			dialog.text = "Это всё, что я должен был передать, месье.";
			link.l1 = "Я вам что-то должен за вашу услугу?";
			link.l1.go = "Seabattle_pirate_4";
		break;
		
		case "Seabattle_pirate_4":
			dialog.text = "Нет, своё вознаграждение я уже получил. Удачи, месье.";
			link.l1 = "";
		...
		case "Seabattle_pirate_6":
			...
			AddQuestRecord("Sharlie", "5-36");
		break;
		// пират-обманщик

		case "Seabattle_pirate_a":
			dialog.text = "Прошу прощения, ведь это вы - "+GetFullName(pchar)+"?";
			link.l1 = "Именно так. Чем обязан вашему вниманию, сударь?";
			link.l1.go = "Seabattle_pirate_1a";
		break;
		
		case "Seabattle_pirate_1a":
			dialog.text = "Позвольте представиться - "+GetFullName(npchar)+". Меня послал к вам Чезаре Крейг. Вы, должны его знать - это хозяин таверны в Ле Франсуа. Он сказал, что вы ожидаете важного сообщения, и у него есть нужная вам информация.";
			link.l1 = "Хорошо, передайте ему, что как только корабль будет готов к отплытию, мы немедленно отправимся к берегам пиратского поселения на другой стороне острова.";
			link.l1.go = "Seabattle_pirate_2a";
		break;
		
		case "Seabattle_pirate_2a":
			dialog.text = "Да, он просил поторопиться, отправляться надо немедленно, потом в этом уже не будет никакого смысла. И ещё, месье де Мор. Чезаре сказал, что формальным поводом для вашего появления в Ле Франсуа должна стать доставка партии вина на Гваделупу. Он сейчас как раз активно этим занимается, поэтому ваш визит не вызовет никаких подозрений.";
			link.l1 = "Что-то ещё?";
			link.l1.go = "Seabattle_pirate_3a";
		break;
		
		case "Seabattle_pirate_3a":
			dialog.text = "Это всё, что я должен был передать, месье.";
			link.l1 = "Я вам что-то должен за вашу услугу?";
			link.l1.go = "Seabattle_pirate_4a";
		break;
		
		case "Seabattle_pirate_4a":
			dialog.text = "Нет, своё вознаграждение я уже получил. Удачи, месье.";
			link.l1 = "";
			link.l1.go = "Seabattle_pirate_6a";
		break;
		
		case "Seabattle_pirate_5a":
			dialog.text = "Вот и славно! Тогда я пойду за своей тысячей. Спасибо, кэп!";
			link.l1 = "Да не за что...";
			link.l1.go = "Seabattle_pirate_6a";
		break;
		
		case "Seabattle_pirate_6a":
			DialogExit();
			LAi_SetActorType(npchar);
			LAi_ActorRunToLocation(npchar, "reload", "gate_back", "none", "", "", "Seabattle_pirate_open", -1);
			AddDialogExitQuest("FirstOut_8"); 
			//pchar.questTemp.Sharlie = "seabattle";//в Ле Франсуа
			//AddQuestRecord("Sharlie", "11");
			AddQuestRecord("Sharlie", "5-36");
		break;
		...
		case "UsurerMan_1":
			dialog.text = "Капитан Шарль де Мор?";
			link.l1 = "Да, месье. Прошу прощения, но я несколько тороплюсь....";
			link.l1.go = "UsurerMan_2";
		break;

		case "UsurerMan_2":
			dialog.text = "Я помощник местного ростовщика и должен передать вам важное сообщение на словах. Бухта Ле Марен через пару часов.";
			link.l1 = "Чёрт! Как не во время! (Шарль понимает, что ему придётся делать нелёгкий выбор между погоней за оставшейся бандой по наводке человека Валинье и ликвидациёй должника ростовщика чтобы получить долговые расписки начальника тюрьмы и освободить Дюгари)";
			link.l1.go = "UsurerMan_3";
		break;

		case "UsurerMan_3":
            dialog.text = "Что мне передать моему боссу?";
            link.l1 = "(Конечно же, самое важное не упустить ублюдков и не подвести Валинье и его человека в Ле Франсуа) Передайте, что мне очень жаль, но я не смогу выполнить свою часть уговора, так как сейчас обременён крайне неотложным делом.";
            link.l1.go = "UsurerMan_exit1";
	        link.l2 = "(Но какое-то необъяснимое чутьё подсказывает Шарлю, что нельзя бросать дело Дюгари, и он готов положиться на это чутьё даже против очевидной логики) Хорошо. Передайте ему, что я немедленно собираю людей и выдвигаюсь в указанную бухту.";
	        link.l2.go = "UsurerMan_exit2";
        break;  

		case "UsurerMan_exit1":
	    AddDialogExitQuest("FirstOut_10a"); 
		pchar.questTemp.Sharlie = "seabattle";//в Ле Франсуа
        dialogexit();
		AddQuestRecord("Sharlie", "5-40");
        break; 

		case "UsurerMan_exit2":
	    AddDialogExitQuest("FirstOut_10"); 
        dialogexit();
		AddQuestRecord("Sharlie", "5-41");
        break; 

        // бандиты на берегу
		case "FirstOutBandit1_1":
			dialog.text = "Монсеньор, корабль готов к выходу в море. Мы ожидаем вашей команды.";
			link.l1 = "Хорошо, начинайте погрузку.";
			link.l1.go = "FirstOutBandit1_exit";
		break;
		

		case "FirstOutBandit1_exit":
	    AddDialogExitQuest("FirstOut_15"); 
        dialogexit();
        break;

		case "FirstOutBandit_1":
			dialog.text = "Монсеньор, у нас 'гости'! Это де Мор со своей командой!";
			link.l1 = "Откуда ему здесь взяться?! Он же должен сейчас быть в другом месте....";
			link.l1.go = "FirstOutBandit_exit";
		break;
		

		case "FirstOutBandit_exit":
	    AddDialogExitQuest("FirstOut_17"); 
        dialogexit();
        break;


		case "SenPierDoctor_1":
			dialog.text = "Ну что, месье, теперь вам достаточно полегчало, чтобы вы могли наконец-то покинуть эти стены.";
			link.l1 = "Спасибо, доктор, я вам крайне признателен за то, что так быстро смогли поставить меня на ноги.";
			link.l1.go = "SenPierDoctor_exit";
		break;


		case "SenPierDoctor_exit":
	    AddDialogExitQuest("FirstOut_26"); 
        dialogexit();
        break;


		case "LucilDoctor_1":
			dialog.text = "Я очень рада, Шарль, что вы уцелели в этой...жуткой суматохе. Не знаю, смогу ли я когда-либо заслужить ваше прощение, но знайте, что я всегда готова выполнить любую вашу просьбу.";
			link.l1 = "Мне рассказали, что именно благодаря вам смогли быстро найти и привести доктора, Люсиль. Я благодарен за ту роль, которую вы сыграли в моём воскрешении, хе-хе. Что до прошлого, то многим из нас приходится играть совсем не те роли, которые мы бы хотели. Остаётся лишь рассчитывать, что в следующий раз мы окажемся на одной стороне.";
			link.l1.go = "LucilDoctor_2";
		break;


		case "LucilDoctor_2":
			dialog.text = "Я тоже очень на это надеюсь, месье де Мор. Берегите себя.";
			link.l1 = "Я уже говорил, что для вас - просто Шарль, Люсиль. Всегда буду рад снова видеть вас.";
			link.l1.go = "LucilDoctor_exit";
		break;

		case "LucilDoctor_exit":
	    AddDialogExitQuest("FirstOut_27"); 
        dialogexit();
        break;
