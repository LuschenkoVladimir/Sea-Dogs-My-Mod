void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
		    ...
		    if (CheckAttribute(pchar, "questTemp.Trustwothy_7") && pchar.location == "Marigo_town")
            {
				dialog.text = "Хм. Дверь в дом Глонгов закрыта. Возможно, Лора ушла на прогулку или легла спать. Пойду на корабль и решу, что делать дальше.";
				link.l1 = "";
				link.l1.go = "exit";
				DeleteAttribute(pchar, "questTemp.Trustwothy_7");
				AddDialogExitQuest("Trustwothy_8");
			}
		    ...
		    if (CheckAttribute(pchar, "pchar.questTemp.DeMor_2") && pchar.location == "baster_town")
            {
				dialog.text = "Чёрт бы побрал моего братца Мишеля. Из-за него я оказался в этом захолустье.";
				link.l1 = "";
				link.l1.go = "exit";
				DeleteAttribute(pchar, "pchar.questTemp.DeMor_2");
				AddDialogExitQuest("DeMor_3");
			}
...
//navy <--
		...
		case "demor_dream":
		dialog.text = "Сойдя с корабля, Шарль сразу же обратил внимание на махавшего ему рукой худощавого господина. Это обстоятельство или лёгкий утренний бриз заставили его организм несколько протрезветь. Он автоматически проверил свои карманы, удостоверившись, что ничего не забыл. Прежде всего, это было письмо его отца и...записка Лулу. Она сохранила аромат её духов, и Шарль не смог удержаться от того, чтобы предаться мимолётным воспоминаниям, которые так манили его, и от которых он так старательно пытался затеряться среди бутылок бургундского...";
		link.l1 = "";
		link.l1.go = "demor_dream_exit";
		break;


		case "demor_dream_exit":
		AddDialogExitQuest("DeMor_3");
		AddDialogExitQuest("Memories_1");
		DialogExit();
		break;
		...
		case "demor_dream_2":
		dialog.text = "Чёрт, где я? Не могу даже пошевелиться. Голова болит и в глазах темно. Крепко меня приложили. Но кто и зачем? Дьявол! Деньги Мазарини! Но зачем они меня приволокли сюда? И откуда они узнали? Ладно, все вопросы потом, а сейчас надо выбираться отсюда. Но как? Я крепко связан по рукам и ногам. Во рту кляп. А один из них сидит прямо здесь. (Шарль делает резкое усилие чтобы выгнуться и освободиться от верёвок и снова теряет сознание)";
		link.l1 = "";
		link.l1.go = "demor_dream_exit_2";
		break;


		case "demor_dream_exit_2":
		AddDialogExitQuest("DeMor_23");
		DialogExit();
		break;
		...
		case "demor_basterdream":
		dialog.text = "Тогда немедленно отправляюсь в таверну.";
		link.l1 = "";
		link.l1.go = "demor_basterdream_exit";
		break;


		case "demor_basterdream_exit":
		AddDialogExitQuest("BasterEvent_11");
		DialogExit();
		break;
       ...
       ////////////////////////////////////////////// Воспоминания о Лулу  ////////////////////////////////////////
		...
		case "demor_Memories":
		dialog.text = "Чувствую себя последним подлецом из-за того, что приходится обманывать любимую женщину. С другой стороны, если бы не вмешательство Мазарини и мадам де Рамбуйе....Именно в её салоне я познакомился с Луизой де Ла Файет. Ничего не имею против интересов Мазарини, Короля и Франции, но теперь я хотел бы уничтожить маркиза только по одной причине - чтобы его тень больше никогда не стояла между мной и Лулу. Ради нашего с ней счастья я действительно готов на всё.";
		link.l1 = "";
		link.l1.go = "demor_Memories_exit";
		break;


		case "demor_Memories_exit":
		AddDialogExitQuest("Memories_9");
		AddDialogExitQuest("Memories_10");
		DialogExit();
		break;


		case "demor_Memories_1":
		dialog.text = "Всё, дело сделано - письмо с печатью Мальтийского ордена у меня...Чёрт! Что это за шум внизу?! Надо срочно уходить отсюда!";
		link.l1 = "";
		link.l1.go = "demor_Memories_exit1";
		break;


		case "demor_Memories_exit1":
		AddDialogExitQuest("Memories_13");
		AddDialogExitQuest("Memories_14");
		DialogExit();
		break;


		case "demor_Memories_2":
		dialog.text = "Нееет!... Лулу! (встаёт на колени перед телом Лулу и бесшумно плачет, пока из-за ран не теряет сознание)";
		link.l1 = "";
		link.l1.go = "demor_Memories_exit2";
		break;


		case "demor_Memories_exit2":
		AddDialogExitQuest("Memories_47c");
